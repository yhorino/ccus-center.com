<link rel="stylesheet" href="/ccus-test/wp-content/themes/cocoon-child-master/tmp/sidemenu.css" type="text/css">

<nav id="common_sidemenu">
 
 <div class="sm_outer ccus">
  
  <div class="sm_header">
   
   <div class="sm_logo">
    <a href="/">
     <div class="sm_logo_button_sp"><img src="/ccus-test/wp-content/uploads/2023/12/logo_ccus.png" alt="全国建設キャリアアップ登録センター"></div>
    </a>
   </div>
   
   <div class="sm_spacer">
   </div>
   
   <div class="sm_menuclose">
    <a onclick="menu_slideout();">
     <div class="sm_menuclose_button_pc"><i class="fas fa-times menuclose_icon"></i> <span class="menuclose_text">閉じる</span></div>
     <div class="sm_menuclose_button_sp sp_button"><i class="fas fa-times menuclose_icon sp_icon"></i><span class="menuclose_text sp_text">閉じる</span></div>
    </a>
   </div>
   
  </div>
  
  <div class="sm_body">
   <ul class="sm_list">
	   <li><a href="/?cat=4">CCUSマガジン</a></li>
   </ul>
  </div>
  
  <div class="sm_footer">
	  
   <div class="sm_footer_telbutton">
    <div class="sm_tel">
     <a href="tel:0120968631">
      <div class="sm_tel_button_pc"><img src="/ccus-test/wp-content/uploads/2023/12/top_tel_ccus_s.png" alt="0120968631"></div>
     </a>
    </div>
   </div>
   
   <div class="sm_footer_buttons">
    <div class="sm_form">
     <a href="/form/">
      <div class="sm_form_button_pc"><i class="fas fa-pen form_icon"></i> <span class="form_text">Webで申込み</span></div>
     </a>
    </div>
	  
   </div>
   
  </div>
  
 </div>
 
</nav>

<script>
 jQuery(function($){
  $('#common_sidemenu').addClass('hide');
 });
 function menu_slidein(){
  jQuery('#common_sidemenu').removeClass('hide');
  jQuery('#common_sidemenu').addClass('show');
 }
 function menu_slideout(){
  jQuery('#common_sidemenu').removeClass('show');
  jQuery('#common_sidemenu').addClass('hide');
 }
</script>

